...
interface EditProductPageComponentProps {
  categories: any;
  fetchProduct: any;
  updateProductApiRequest: any;
  reduxDispatch: any;
  saveAttributeToCatDoc: any;
  imageDeleteHandler: any;
  uploadHandler?: any;
  uploadImagesApiRequest: any;
  uploadImagesCloudinaryApiRequest: any;
}
...
const EditProductPageComponent = ({
  ...
  const [updateProductResponseState, setUpdateProductResponseState] = useState<any>({
    message: "",
    error: "",
  });
  const [attributesFromDb, setAttributesFromDb] = useState<any>([]); // for select lists
  const [attributesTable, setAttributesTable] = useState<any>([]); // for html table
  ...
  const attrVal = useRef<any>(null);
  const attrKey = useRef<any>(null);
  const createNewAttrKey = useRef<any>(null);
  const createNewAttrVal = useRef<any>(null);
        ...
        <Col md={6}>
          ...
          <Form noValidate validated={validated} onSubmit={handleSubmit} onKeyDown={(e) => checkKeyDown(e)}>
            ...
            <Form.Group controlId="formFileMultiple" className="mb-3 mt-3">
              ...
              <Form.Control  type="file" multiple onChange={(e:any) => {
